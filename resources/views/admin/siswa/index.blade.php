...
$(function(){
    const table=$('#table').DataTable({processing:true,serverSide:true,ajax:'{{ route('siswa.index') }}',columns:[
        {data:'DT_RowIndex',orderable:false,searchable:false,className:'text-center',width:'45px'},{data:'nama_lengkap'},
    ...
    $('#downloadClassForm').on('submit',function(){bootstrap.Modal.getOrCreateInstance(document.getElementById('downloadModal')).hide();});
