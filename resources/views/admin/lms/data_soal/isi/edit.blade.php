        ...
        <section class="section">
            <div class="card">
                <div class="card-header bg-primary mb-4 d-flex align-items-center">
                    <h3 class="font-weight-bold text-xl text-white m-0">Edit Isi Soal {{ $data->nama }}</h3>
                </div>
                <div class="card-body">
                    <form id="formData">
                        @csrf
                        <input type="hidden" id="primary_id" name="primary_id" value="{{ $list->id }}">

                        <div class="row mb-3">
                            <label for="soal" class="col-sm-2 col-form-label">Soal</label>
                            <div class="col-sm-4">
                                <textarea name="soal" id="soal" class="form-control" rows="4">{{ $list->soal }}</textarea>
                            </div>
                        </div>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-8">
                                <div class="img-thumbnail mb-2 d-flex align-items-center justify-content-center"
                                    ...
                                    @if ($list->foto)
                                        <img src="{{ asset('assets/bahan_soal/' . $list->foto) }}" alt="Foto Soal"
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-4">
                                <select class="form-select select-jenis" name="jenis" id="jenis">
                                    <option value=""></option>
                                    <option value="1" {{ $list->jenis == 1 ? 'selected' : '' }}>Single Choice</option>
                                    <option value="2" {{ $list->jenis == 2 ? 'selected' : '' }}>Multiple Choice
                                    </option>
                                    <option value="3" {{ $list->jenis == 3 ? 'selected' : '' }}>Text</option>
                                    <option value="4" {{ $list->jenis == 4 ? 'selected' : '' }}>Select</option>
                                    <option value="5" {{ $list->jenis == 5 ? 'selected' : '' }}>Rating</option>
                                </select>
                            </div>
                        </div>

                        <div class="row mb-3" id="row-rating" style="display: none;">
                            ...
                            <div class="col-sm-2">
                                <input type="text" id="jumlah_rating" name="jumlah_rating"
                                    ...
                                    value="{{ isset($detail[0]) ? $detail[0]->jawaban : '' }}">
                            </div>
                        </div>

                        <div class="row mb-3 mt-3" id="row-jawaban" style="display: none;">
                            <div class="col-sm-10 offset-sm-2">
                                ...
                                <div class="mt-3">
                                    <button type="button" class="btn btn-primary tambah-baris">
                                        <i class="bi bi-plus-lg me-1"></i>Tambah Jawaban
                                    </button>
                                </div>
                            </div>
                        </div>
...
@push('scripts')
    <script>
        ...
        $('#formData').on('submit', function(e) {
            ...
            let url = '{{ route('data-soal.isi-update', ['id' => ':id']) }}'.replace(':id',
                id);
            let method = 'PUT';
            ...
            $.ajax({
                ...
                processData: false,
                success: function() {
                    sessionStorage.setItem('success', 'Soal berhasil diupdate!');
                    window.location.href = "{{ route('data-soal.show', ':id') }}".replace(':id',
                        dataid);
                },
                error: function(xhr) {
                    if (xhr.status === 422) {
